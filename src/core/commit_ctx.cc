...
void CommitCtx::NewCommit(std::string * psValue, StateMachineCtx * poStateMachineCtx, const int iTimeoutMs)
{
    m_oSerialLock.Lock();
...
bool CommitCtx::IsMyCommit(const uint64_t llInstanceID, const std::string & sLearnValue,  StateMachineCtx *& poSMCtx)
{
    m_oSerialLock.Lock();
